...
@info: 使用ST-GCN训练行为评分模型, 替换数据为更符合真实的24类数据, 源自50个视频, benchmark, cls model + 24 reg model
@info: Train the action scoring model by ST-GCN model, benchmark, cls model + 24 reg model.
"""
import argparse
import datetime
import os.path
...
from STGCN import ST_GCN, ST_GCN_split, ST_GCN_split_plus, ST_GCN_W, ST_GCN_L, ST_GCN_MLPs, ST_GCN_DeepMLPs, \
    ST_GCN_DeepBackbone, ST_GCN_multi_reg, \
...
def choose_model(opt):
    ...
    #                                      hop_size=opt.hop_size).to(device)
    # elif opt.model == "ST_GCN_DWBackbone_MLPs":
    #     model = ST_GCN_DWBackbone_MLPs(num_classes=opt.classes, in_channels=3, t_kernel_size=opt.t_kernel_size,
    #                                    hop_size=opt.hop_size).to(device)
