# -*- coding: utf-8 -*-
"""
@Time: 2023/8/21 16:12
@Auth: divided7
@File: 数据增强.py
@E-mail: divided.by.07@gmail.com
@info: 使用ST-GCN训练行为评分模型, 替换数据为更符合真实的24类数据, 源自50个视频
@info: Train the action scoring model by ST-GCN model.
"""
import argparse
import datetime
import os.path
...
# from STGCN import *
from STGCN import ST_GCN, ST_GCN_split, ST_GCN_split_plus, ST_GCN_W, ST_GCN_L, ST_GCN_MLPs, ST_GCN_DeepMLPs, \
    ST_GCN_DeepBackbone, ST_GCN_only_reg, ST_GCN_DWBackbone_MLPs_only_reg, Siamese_ST_GCN_DWBackbone_MLPs_only_reg, \
    ST_GCN_DeepBackbone_MLPs, ST_GCN_DWBackbone_MLPs, ST_GCN_DWBackbone_MLPs_split, ST_GCN_DWBackbone_MLPs_split_plus
...
def opt():
    ...
    parser.add_argument("--model", type=str, default="Siamese_ST_GCN_DWBackbone_MLPs_only_reg",
                        help="ST_GCN, ST_GCN_W, ST_GCN_L, ST_GCN_MLPs, ST_GCN_DeepMLPs, ST_GCN_DeepBackbone,ST_GCN_DWBackbone_MLPs,ST_GCN_DWBackbone_MLPs_split")
    parser.add_argument('--load_weight', type=str, default=None, help="预训练权重") # 训练日志/第二式分数回归_暹罗网络BMSE/2023-12-20 15:59:40/stgcn_epoch=468_loss=0.001957362177091892.pt
    ...
    try:
        ...
        pass
    logging.basicConfig(filename=args.output + '/{}.log'.format(datetime.datetime.now().replace(microsecond=0)), level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
